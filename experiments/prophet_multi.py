...
def generate_model_name(params):
    if 'country_holidays' in params:
        model_name = 'daily_prophet_multi_weekdays-{}_diff-{}_hol-{}'.format(
            params['only_weekdays'],
            params['is_differenced'],
            params['country_holidays']
        )
    else:
        model_name = 'daily_prophet_multi_weekdays-{}_diff-{}'.format(
            params['only_weekdays'],
            params['is_differenced'],
        )
...
def generate_model(params):
    if 'country_holidays' in params:
        model = Prophet(
            country_holidays=params['country_holidays']
        )
    else:
        model = Prophet()
...
def run_tests():
    for dataset_name, test_split_before in DATASET_LIST:
        ...
        if dataset_name == 'open_source_ballarat_daily_waste_2000_jul_2015_mar':
            params['country_holidays'] = 'AUS'
        elif dataset_name == 'open_source_austin_daily_waste_2003_jan_2021_jul':
            params['country_holidays'] = 'US'
